import { facilities } from "@/app/utils/data";
import { Minus } from "@/app/utils/Icons";
import { FacilityKey, FacilityProps } from "@/app/utils/type";
import React, { useState } from "react";

const Facilities: React.FC<FacilityProps> = ({
  facilities: facilityStates,
  onHandleFacilityChange,
}) => {
  ...
  return (
    <div className="p-6 mb-4 rounded-xl border border-[#E6E9ED] bg-white">
      <div className="flex justify-between items-center  pb-3.5 border-b border-[#E5E9EE] mb-3">
        <h3 className="text-xl font-semibold text-[#252B37]">Amenities</h3>
        <Minus />
      </div>
      <div className="space-y-3">
        {visibleFacilities.map(({ key, label }) => {
          const typedKey = key as FacilityKey;
          return (
            <label
              key={typedKey}
            ...
            >
              <input
                type="checkbox"
                checked={facilityStates[typedKey]}
                onChange={() => onHandleFacilityChange(typedKey)}
                className="hidden peer"
              />
              <span
                className={`w-4 h-4 rounded-[4px] flex items-center justify-center 
          bg-white  ${
            facilityStates[typedKey] ? "border-0 " : "border-2"
          } border-gray-300`}
              >
                {facilityStates[typedKey] && (
                  <svg
        ...
        {!expanded && facilities.length > 3 && (
          <div className="pt-2">
            <button
              onClick={() => setExpanded(true)}
              className="text-[#6C0443] underline text-sm font-medium hover:text-[#6a004b] transition-colors"
            >
              see more
            </button>
          </div>
